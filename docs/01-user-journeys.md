...
- Peer reconnects but with a fresh app install (new identity) → this "looks like" the same peer only if we let display name stand in for identity, which Journey 2 already ruled out. The old pending messages correctly fail to deliver to the new identity, and that's correct behavior, not a bug. This is worth being explicit about so it isn't accidentally "fixed" into matching on display name later.
