...
**Update, 2026-09-09, the experimental path was actually tried:** two API 34 AVDs (emulator 37.1.11.0), both running the app. Every individual BLE API call succeeded cleanly on both sides, confirmed via Logcat: `BluetoothGattServer.onServerRegistered` and `onServiceAdded` both returned `status=0` for both the identity and messaging services on both emulators, `BluetoothLeScanner.onScannerRegistered` returned `status=0` on both, and advertising reported no failure on either (`BlePeripheralRole`'s own failure-path log never fired). Neither device ever appeared in the other's Nearby list. This is a clean result, not an inconclusive one: it confirms the app's own BLE code is exercising every API surface correctly, and isolates the actual gap to the two emulator instances never being bridged at the radio level, exactly the outcome this section already flagged as plausible. Real physical devices remain the only confirmed path to verifying actual discovery.
