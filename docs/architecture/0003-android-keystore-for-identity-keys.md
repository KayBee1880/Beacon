...
- Key usage goes through the Keystore's API surface rather than a general-purpose crypto library for this specific key. This is acceptable since this key's only job is proving device identity, not general message encryption (that's Milestone 3's `CryptoService`, a separate decision).
