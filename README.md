...
**What this deliberately does not claim:** there are zero automated tests: everything verified so far was verified manually, by hand, on one emulator, and only covers Milestones 0 and 1. `Conversation` and `Message` (the rest of the Milestone 1 domain model) have no UI exercising them beyond what Milestones 3 and 4 added. Milestones 2 through 9's BLE/Wi-Fi Direct/crypto/retry/relay code compiles cleanly but has never actually run a discovery, a chat, a retry, a relayed message, or a file transfer, and the new Mesh diagram has never been seen rendered against a real, populated peer list. Real device testing is expected to surface real bugs, per the same honesty standard Milestone 1's JDK/build issues were tracked under. All of the above are real, tracked roadmap items, not silent gaps.
...
- [x] **Milestone 9: Visualization & connection quality UX** ← current (design + code written; logical topology diagram, no location permission; needs real multi-peer scenarios to verify layout)
...
│   └── app/src/main/
│       ├── java/com/beacon/              # BeaconApplication, MainActivity, ChatScreen, ConversationsScreen, MeshScreen
